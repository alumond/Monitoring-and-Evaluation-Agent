...
def evaluate_decision_policy(
    ...
    for indicator in analytics.get("kpi", {}).get("performance", []):
        ...
        _append_issue(
            ...
            details=(
                f"Indicator performance is {performance:.1f}% against the configured "
                f"{settings.escalation_kpi_threshold:.1f}% threshold."
            ),
            evidence=indicator,
...
def build_decision_notification(
    ...
    subject = f"{label} - {settings.project_name} - {period}"
    lines = [
        f"{label}: {settings.project_name}",
        f"Date: {period}",
        "",
